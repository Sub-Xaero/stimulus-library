...
export class ElementSaveController extends BaseController {
  ...
  clear(event?: Event) {
    ...
    this._store.clear();
    this.dispatchEvent(this._element, `${this.identifier}:cleared`);
  }
  ...
  save(event?: Event) {
    ...
    this.dispatchEvent(element, `${this.identifier}:save:success`);
  }
  ...
  restore(event?: Event) {
    ...
    if (!this._store.isEmpty()) {
      ...
      this.dispatchEvent(element, `${this.identifier}:restore:success`);
    } else {
      this.dispatchEvent(element, `${this.identifier}:restore:empty`);
    }
  }
