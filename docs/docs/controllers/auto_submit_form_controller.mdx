...
| `eventMode` (Optional) | String | The type of event to monitor the form for before submitting, either `change`, `input` or `debounced`. <br/><br/>`change` will monitor all inputs for `change` events, i.e. when the user clicks away from a changed input.<br/><br/>`input` will monitor all inputs for `input` events, i.e. every keypress the user types into a form input.<br/><br/>`debounced` will monitor all inputs for `input` events but only submits the form after `debounceInterval` milliseconds has elapsed after the last `input` event fires, i.e.  when the user stops typing for 2 seconds | `change` |
| `debounceTimeout` (Optional) | String | The duration in milliseconds before `debounced` `input` events trigger a form submission. | `2000` |
...
- If the `eventModeValue` is `request` and the browser does not support `.requestSubmit()`, then the controller will use any existing submit buttons, or create a hidden submit button (which will be cleaned up on 'disconnect'), to submit the form by clicking on them.
